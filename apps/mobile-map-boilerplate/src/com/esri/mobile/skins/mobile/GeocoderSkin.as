...
public class GeocoderSkin extends com.esri.mobile.skins.mobile.StageTextInputSkin
{
    ...
    {
        // ycabon: Skip super to do it ourself
        // super.layoutContents(unscaledWidth, unscaledHeight);

        var unscaledTextWidth:Number = Math.max(0, unscaledWidth - 4 * layoutGap);
        var unscaledTextHeight:Number = Math.max(0, unscaledHeight - 2 * layoutGap);

        var clearButtonHeight:Number = unscaledHeight - layoutBorderSize - 2 * layoutGap;
        if ((clearButton && clearButton.visible) || (busyIndicator && busyIndicator.visible))
        {
            unscaledTextWidth -= layoutGap + clearButtonHeight;
        }
        ...
        if (textDisplay)
        {
            ...
            setElementPosition(textDisplay, layoutGap, textY);
        }
        ...
        if (promptDisplay)
        {
            ...
            var promptY:Number = Math.round(0.5 * (unscaledTextHeight - promptHeight)) + layoutGap;

            setElementSize(promptDisplay, unscaledTextWidth, promptHeight);
            setElementPosition(promptDisplay, layoutGap, promptY);
        }

        if (clearButton)
        {
            setElementPosition(clearButton, unscaledWidth - clearButtonHeight - layoutBorderSize - layoutGap, layoutGap);
            setElementSize(clearButton, clearButtonHeight, clearButtonHeight);
        }
        if (busyIndicator)
        {
            setElementPosition(busyIndicator, unscaledWidth - clearButtonHeight - layoutBorderSize - layoutGap, layoutGap);
            setElementSize(busyIndicator, clearButtonHeight, clearButtonHeight);
        }

        resultList.width = unscaledWidth - layoutGap;
    }
